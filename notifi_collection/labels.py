...
LABEL_SPECS: tuple[LabelSpec, ...] = (
    ...
    LabelSpec(
        ...
        (
            ...
            ("c-c+0.8", "지정 측으로 돌아눕는다."),
            ("c+0.8-c+1.8", "상체를 세우고 두 다리를 내려 침대 가장자리에 앉는다."),
            ("c+1.8-c+2.8", "양발로 바닥을 밀어 일어나려 하지만 균형을 잃고 지정 측으로 내려간다."),
            ("c+2.8-c+4.0", "지정 측 엉덩이와 전완이 순서대로 침대 옆 매트에 닿도록 통제된 낙상을 수행한다."),
            ("c+4.0-10.0", "침대 옆 매트에서 옆으로 누운 자세를 유지한다."),
        ),
        "침대 옆 매트 측와위",
        "완전히 일어난 뒤 걷기, 침대에서 굴러 떨어지기, 머리부터 떨어지기, 침대 또는 벽을 잡기",
        "일어나려는 시도 없이 바로 바닥으로 내려감, 매트 밖 접촉, 머리 또는 손목 충격, 안전요원 접촉",
        ("bed_exit_fall_left", "bed_exit_fall_right"),
    ),
    LabelSpec(
        "D04",
        "DANGER",
    ...
    LabelSpec(
        ...
        10,
        "danger",
        "S03과 같은 착석 자세. 지정 낙상 측 의자 옆을 이중 매트로 덮고 의자를 고정한다.",
        (
            ("0.0-c", "정지 앉기를 유지한다."),
            ("c-c+0.6", "몸통을 약 15도 앞으로 기울이고 일어서기 동작을 시작한다."),
            ("c+0.6-c+1.6", "양발로 바닥을 밀어 엉덩이를 띄우지만 균형을 잃고 지정 측으로 내려간다."),
            ("c+1.6-c+2.8", "지정 측 엉덩이와 전완이 순서대로 의자 옆 매트에 닿도록 통제된 낙상을 수행한다."),
            ("c+2.8-10.0", "의자 옆 매트에서 옆으로 누운 최종 자세를 유지한다."),
        ),
        "의자 옆 매트 측와위",
        "의자 전도, 완전히 선 뒤 걷기, 머리 또는 손목 첫 접촉, 매트 밖 착지",
        "일어서기 시도 없이 옆으로 미끄러짐, 의자가 2 cm 이상 움직임, 안전요원 접촉, 매트 밖 착지",
        ("chair_exit_fall_left", "chair_exit_fall_right"),
    ),
)
